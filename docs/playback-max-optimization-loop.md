...
| 4 | x2 Sharp | M16-1327 | 11.0 | 68.7 | 170 | content OK; span 40.8 SMOOTH (PASS) | 17.2 | wb-f917930d2e804510 (quarter-dim processing path 11) |
| 5 | x2 Sharp | M16-1347 | 11.0 | 65.4 | 154 | content OK (PASS) | 17.2 | wb-f917930d2e804510 (quarter-dim processing path 11) |
| 6 | x2 Sharp | M16-1446 | 10.9 | 70.3 | 155 | content OK (PASS) | 16.8 | wb-f917930d2e804510 (quarter-dim processing path 11) |
...
**R4-6 LANDED (user chose "x1 full"):** before reaching for the parity-risky SIMD rewrite,
a lower-risk bigger-lever surfaced - applyProcessingObject8's `force_export_policy=1` was an
over-broad R4-2 band-aid suppressing the cheap preview quarter-res SH blur for ALL playback
direct8 lanes (the real R4-2 corruption was the TLS self-alias, fixed separately). Lifting it
for playback (exports stay full quality) landed full-quality x1 10.6->11.6 / 11.2->11.9
(+6-9%, 30s sustained), x2 toward native, x1 Auto +3-4%; tonally identical (same-frame Delta
~0.6/255 uniform), all lanes VIEWED clean, suite green. The SIMD-rewrite lever
(core_math + SH RBF intrinsics, ~14-16 fps) remains OPEN and un-started - higher risk, now
on a faster baseline (~11.6-11.9). Merged wb-52e74c73a8b74f2b.
...
gains possible."** Queue = the verified six-dimension audit findings
(`.claude-state/profiling/20260612-opportunity-audit.json`), executed as brokered work
blocks R4-0 .. R4-5 with full gates + VIEWED visual sweeps each.

**Stop condition reached:** x2/x4/x8 are NATIVE-LOCKED (medians 22.7-23.8 against the
23.976 ceiling — no further gain is possible by definition), and x1's remaining levers are
all below single-clip noise (LJ92 refill + SIMD gaps ≈2-3ms against a ~40ms render bound by
dual-ISO recon + half-dim processing, with raw reads already 95%+ prefetch-hidden at
1.7-4.1ms). The remaining x1 speed steps are the SHIPPED user-selectable tradeoffs
(Quarter ~17, aggressive ~16-19).

**Per-item outcomes:**
- R4-0/0b/0c (debt): machine-checked expected-failures manifest (18->10 names); ALL EIGHT
  [D8] direct8 parity tests fixed FOR REAL (intrin pre_calc_levels skip = mean-37 levels
  error default-on since 2026-05-30; Reinhard green copy-paste; one-sided SH blur policy);
  exports un-preview-forced; look-assist UAF + 1/36 race fixed; cpu_summary %100+ bug fixed.
- R4-1 KEEP (merged 2c5017e5): contiguous-window raw prefetch (hit rate 60% -> 95%+,
  raw read 8-9 -> 1.7-4.1ms) + frame-signature de-dup (full ~1.1MB hash 1x/frame, was 2-3x);
  processed16 mean -13.4%; fps pacing-bound at the time (subsequently unlocked by R4-3).
- R4-2 KEEP (merged a3deba5a): direct8 unlocked for non-neutral SHARP-preview receipts
  (the look-assist class) -> x2 Sharp 17.6 -> 21.6 mean (+15/+30/+23% per clip, honest
  clean-render A/B); THE VIEWED SWEEP CAUGHT THREE LATENT BUGS the fps/content detectors
  were blind to (TLS rgb_u16 self-alias rendering near-flat x2 direct8 frames, latent since
  R3-5; quarterres-SH blur content the kernel misreads; in-place writes to the live
  presented buffer) — all fixed; lane economics scoped via mlv_preview_direct8_input_is_cheap.
- R4-3 KEEP (merged 90451965): pipelining = ONE edit (drawFrameReady dispatches the next
  render BEFORE the present span; the serialization was GUI-thread request starvation, not
  a lock). x2 21.7->23.8 / 22.2->23.3 / 21.3->22.7 = native-locked; x1 wash (render-bound;
  the audit's x1 +20-30% estimate REFUTED — pacing converts to fps only where the render
  fits the frame budget); flicker=0 frozen=0 on all runs.
- R4-4 BLOCKED-ON-ASSET (not attempted): extending the proxy cores past llrpHQDualIso needs
  a real non-dual-ISO clip for the mandatory visual sweep; every clip on this machine
  (C:\temp\MLV) is dual-ISO (avg_llrawproc_dual_iso_ms ~11 confirms live recon). Queued as
  a roadmap item; ask the user for standard-CFA footage.
- R4-5 quality session: x1 proxy-vs-Full filmstrips VIEWED on fine-detail content (railing
  edges, hair) — NO visible aliasing/fringing/moire at display scale; the audit's
  clump-decimation/chroma-siting/tent-upscale findings stay documented as below-visibility
  pixel-level future work. LJ92 wider refill + SIMD gaps REFUTED-BY-STANDINGS (sub-noise
  against current lane bounds, nothing to gain at native-locked scales).

**FINAL STANDINGS (M16 trio medians, native 23.976; baseline 2026-06-10 -> round-4 close):**
x1 Sharp 5.4-8.1 -> ~16.1-16.7 Auto / ~17 Quarter opt-in · x1 Aggr 7.2 -> ~16-19 ·
x2 Sharp 11 -> 22.7-23.8 (NATIVE-LOCKED, sharper too: direct8 half-res fused replaces
quarter-process+upscale for look-assist receipts) · x2 Aggr ~15.6-22 (wash-to-up) ·
x4 13-13.7 -> 22.7+ (native) · x8 13-15.9 -> 22.7+ (native). Pause/scrub/export always
full quality; every tradeoff is a persisted GUI setting; every proxy path env-kill-switchable.

**Open follow-ups (roadmap, not blockers):** R4-4 non-dual-ISO proxy extension (needs
footage) · SH gate-level dimension fix (raw_processing.c:1260-1284 class; effective-scale
workaround shipped) · below-visibility quality polish (downsample low-pass, Catmull-Rom
upscale) · 2a test-debt chip task_72067605 · [STALE?] manifest pins adjudication (9 names).
...
| 2026-06-12 | x2 Sharp (round-3 item 5, re-ranked from telemetry) | Process at the quarter render dims inside the x2 preview (path 11): no-upscale mode for the x2 quarter core, applyProcessingObject at quarter dims with effective scale 4 (the proven SH x4 lane), upscale the PROCESSED result; MLVAPP_DISABLE_QUARTERRES_X2_PROCESSING=1 restores path 5; scale-2 signature hashes the mode; opt-in x2 prefetch worker gated off while quarter processing is active (byte-coherence). Original R3-5 (aggressive inversion) re-ranked AWAY: its premise dissolved (current x2 Aggr 14.9, render 47.7ms = on par with Sharp). | x2 slow decile is 97.5% processing at ~67ms on a quarter-res render upscaled before processing; same lever that took x1 from ~9 to ~17. Expected x2 ~15 -> toward 20+. | interleaved on/off A/B via the kill switch, 2 reps, 3 clips, Sharp; x1/x4/x8 no-regress; filmstrip pairs + balance trace VIEWED; suite | rows 4-6 target; rest watch | exe SHA at measurement | Holds only if quarter-dim processing under the SH x4 lane is artifact/cast-free at x2 output size (traces + eyes) and the aggressive path stays untouched (path 4 tests). | x2 families all PASS (path 11 default, kill->5 from_raw untouched, aggressive path-4, worker byte-identity under aligned kill) | Interleaved A/B ALL 12 PASS, on-arm wins all six pairs: 1327 17.2/16.1 (+6.8%), 1347 17.2/15.3 (+12.4%), 1446 16.8/15.75 (+6.7%) means; render 37.5-39.8 vs 43.5-49.1 ms; no-regress x1 14.1 / x4 23.3 / x8 24.4 (native); balance trace arms <=0.46; filmstrips SMOOTH; path-11 frame + x8 canary VIEWED clean | KEEP (x2 Sharp ~16.8-17.2 median). Suite adjudication: StandardPreviewScaleTwoUsesQuarterResShadowsHighlightsByDefault failed deterministically post-change (REAL, mine: the x2-specific SH-quarterres telemetry only fires on path 5 now) - test re-pinned to the preserved contract behind the kill switch; final suite = exactly the 18 adjudicated names. | wb-f917930d2e804510 |
| 2026-06-12 | harness + cold-open correctness (round-4 item 0: debt block) | (a) cpu_summary telemetry: QString::arg cannot address %100+ so 13 fields were silently garbage since landing - split into cpu_summary_ext + wrapper merge (first honest read: processed16_cache_store=4.9ms); (b) machine-checked expected-failure manifest (tests/pipeline/expected-failures.txt + tools/testing/check-expected-failures.ps1) replaces the prose baseline - suite gate now fails on ANY new name and reports newly-passing names for pruning; (c) look-assist worker UAF fixed: in-flight counter + drainLookAssistWorkers() before every freeMlvObject (generation bumped first so the drain is normally microseconds); (d) look-assist never-applied race (~1/36) fixed: dropped async results clear the dispatch-time de-dupe marker and log async_dropped, so the next trigger re-dispatches; (e) direct8 byte-identity cluster adjudication (agent report folded in below). | Unblocks R4-2 (direct8 unlock needs a trustworthy parity net); kills a user-reachable crash class and the 1/36 no-look-applied runs; un-blinds the telemetry that hid the R4-1 overhead band. | manifest gate vs the suite; functional smoke (look-assist applies, fps in range); e2e telemetry field check | n/a (debt block) | exe SHA at commit | Holds only if the drain wait stays bounded (worst case = one in-flight findMlvWhiteBalance render) and the manifest is kept pruned (newly-passing names acted on). | gate exit 0 (observed=17 subset of 18, flake surfaced as newly-passing); smoke PASS lookAssistApplied=True 15.2 median; cache_store16=4.906 readable | ADJUDICATION (agent, empirically confirmed): 5 REAL DEFECTS / 3 STALE. Root cause of the mean-37 cluster: the AVX2 intrinsics direct8 kernel (raw_processing_8bit_kernel_avx2_intrin.inc) SKIPS pre_calc_levels (black/white stretch) - default-on since 06976e82 (2026-05-30); with MLVAPP_DISABLE_AVX2_INTRIN_DIRECT8=1 the parity tests pass 0/12.3M pixels. Also: intrin lacks AgX while the preview bypass (raw_processing.c:2301-2304) routes AgX onto it. SEVERITY ESCALATION: video_mlv.c:6218 (716b9ada) forces preview mode for EVERY getMlvProcessedFrame8 caller - H.264/H.265 encode (MainWindow.cpp:8222) and PNG/JPG single-frame export (SingleFrameExportDialog.cpp:109) therefore get intrin (wrong-levels) pixels on direct8-eligible receipts or the x1 HALF-RES PROXY on non-eligible ones, contradicting the never-proxied export guarantee. Activation trio (PhaseE8/E9) = stale expectations of the pre-ef119ce8 contract. Fixes queued as R4-0b (same debt theme): unforce preview for non-playback callers, port pre_calc_levels into the intrin gathers, narrow the AgX bypass, re-pin the trio. | KEEP (debt; no perf claim) | wb-c7f75b7611ee4016 |
| 2026-06-12 | direct8 parity + export semantics (round-4 item 0b) | (a) Unscaled getMlvProcessedFrame8 (exports, single-frame grabs) no longer forces playback-preview semantics - exports can never take the proxy or preview-gated paths again (playbackPreview param threaded through _with_scale; Scaled entry keeps preview ON); (b) the AVX2 intrin direct8 kernel now applies pre_calc_levels (was skipped = the mean-37 whole-frame levels error, default-on since 2026-05-30) in the gather index prep + scalar tail; (c) intrin Reinhard green channel used Reinhard_for_blue instead of ReinhardTonemap_f (copy-paste) - the deterministic ~130k-pixel residual. | Users on AVX2+FMA get correct levels on every direct8-eligible playback frame AND byte-correct 8-bit exports; six of eight [D8] parity tests fixed FOR REAL (not re-pinned). | parity test families; manifest gate; no-regress singles | n/a (correctness) | exe SHA at commit | Holds only if the PhaseE9 SH-blur divergence investigation (in flight) lands a fix or an honest adjudication - the pair stays in the manifest meanwhile. | DirectProcessed8FastPath* 4/4 PASS incl. AVX2 canary; PhaseE7 3/3 PASS; PhaseE8 PASS; suite gate exit 0 (observed 11, was 18); x1 15.2 / x2 18.5 singles clean | six parity fixes verified; E9 pair remains (SH blur source) | KEEP (correctness; no perf claim) | wb-709e3ca635714914 |
| 2026-06-12 | direct8 SH-blur parity (round-4 item 0c) | Extracted the shadows/highlights blur pre-pass into ONE shared helper (processing_compute_shadows_highlights_blur) called by BOTH applyProcessingObject and applyProcessingObject8. Root cause (agent-confirmed): commit 25795f27 (2026-05-31) moved the 16-bit blur policy to half-res for even-dim frames; the 8-bit copy silently stayed full-res - one-sided policy drift = the PhaseE9 1.70M-pixel divergence. Bonus: the direct8 path now gets the half-res RBF perf win it was missing. | ALL EIGHT [D8] parity tests green (fixed for real); the direct8 fast path has full regression coverage again - R4-2 unblocked. | parity families; manifest gate; no-regress singles | n/a (correctness) | exe SHA at commit | Holds only if no other consumer depended on the 8-bit full-res blur (none found: the helper is the only writer of blur_image now). | PhaseE9 2/2 PASS; all 8 [D8] PASS; gate exit 0 (observed=10, manifest=10); singles below | (singles in this cell) | KEEP (correctness + direct8 SH perf win) | wb-9b503a333c234533 |
| 2026-06-12 | x1 + x2 all modes (round-4 item 1: non-pixel quick wins) | (a) Raw-uint16 prefetch worker targets a CONTIGUOUS window (baseFrame+offset) when the observed request stride is <=4 instead of extrapolating the last delta - drop-frame playback advances by an alternating 1..3-frame pattern so a fixed-stride guess missed most next requests (measured 62% hit rate at x1); stride extrapolation kept for genuine fast scrubs (stride>4). (b) Signature de-dup: the playback 8-bit entry's frame signature is passed down into getMlvProcessedFrame16_with_scale (was recomputed), and BOTH store-site full recomputes (indirect processed16_store + the direct8 Phase-E7 store) are narrowed by a cheap render-mutable-state fingerprint (llrawproc status words + RAWI/processing black-white levels - the only hashed state a render can mutate); the full ~1.1MB hash is paid only when the fingerprint actually drifted (once per clip/state change). Wrong-fingerprint failure mode = one redundant cache miss, never a stale hit. | Indirect lane drops from 3 full state hashes per frame to 1; direct8 lane from 2 to 1; prefetch hit rate 62% -> toward 100% hides more of the ~8ms avg raw_uint16 read. Non-pixel change: zero output deltas expected. | interleaved base/r41 exe-swap A/B, x1 Sharp 3 reps per arm + raw prefetch hit-rate cited from the same runs; x2/x4/x8 no-regress singles; suite via manifest gate; NO filmstrip gate (non-pixel: byte-identical output asserted by the full parity/identity test families) | rows 1-6, 13-15 watch | working tree on master 32ecd0c9 (exe SHA at measurement) | Holds only if the fingerprint covers ALL render-mutable hashed state (audited: E7 comment names llrawproc status words; sync_processing_black_white_levels writes processing levels; llrawproc dual ISO rewrites RAWI levels - all fingerprinted; any miss self-heals as a single redundant miss) and the precomputed signature is computed under identical pre-render global state as the callee's own would be (verified: no state writes between the 8-bit entry hash and the 16-bit call). | gate exit 0 (observed=10 = exactly the manifest, incl. the documented ProcessingFilters flake); byte-identity asserted by Processed8Prefetch*MatchesReference + DirectProcessed8FastPath* + PhaseE7/E8/E9 families all PASS at the A/B SHA; all 12 detector verdicts PASS | Interleaved exe-swap A/B base=ACEBF484 vs r41=10E7C4ED, x1 Sharp, detector medians (6 pairs): 1327 14.3->16.4 / 15.2->14.1; 1347 16.4->20.0 (+22%) / 13.7->19.2 (+40%); 1446 18.5->14.9 / 13.9->14.7 (the 18.5 base read rode an anomalous 102-hit prefetch run); arm means 15.33 vs 16.55 (+8%). MECHANISM (all 16 x1 runs): raw_prefetch_hits 65-102 -> 95-155, avg_raw_uint16_ms 6.97-9.28 -> 1.69-4.12 (-65..-75%), avg_processed16_ms mean 42.98 -> 37.23 (-13.4%); no-regress singles (r41, hot machine after 19 consecutive runs): x2 16.9 / x4 21.7 / x8 18.9 all PASS | KEEP (free non-pixel win: the mechanism is consistent and large on every run while the fps median is pacing-bound - +22..+40% median on 1347, wash within single-clip noise on 1327/1446; honest note: the >=15%-fps-on-2/3-clips keeper bar was NOT met, kept on no-risk + mechanism evidence; avg_present_pacing_ms ~12.8 is now a visible bound -> feeds R4-3 pipelining) | wb-f141269aee784bed |
| 2026-06-12 | x1 + x2 Sharp/Aggr (round-4 item 2: direct8 preview unlock) | Remove the four 06976e82/ef119ce8-era preview band-aids in raw_processing.c now that their root causes are fixed (R4-0b/0c: intrin pre_calc_levels skip, Reinhard green copy-paste, one-sided SH blur policy): (a) preview local-tone gate relaxed from neutral-only to the export contract (clarity<0.01 only); (b) SH blur pre-pass no longer preview-suppressed (SH receipts get the shared R4-0c helper); (c) the preview bypass in processing_direct8_requires_shared_kernel REMOVED - preview routes AgX/SH/contrast/vibrance/sat onto the shared C kernel by capability exactly like exports (this also fixes a LIVE bug: neutral AgX receipts in preview ran the AgX-less intrin kernel = silently un-tonemapped pixels); (d) preview serial-render forcing deleted (its receipt classes now run the shared kernel, which exports already run row-partitioned multi-threaded with byte-identity coverage). Look-assist (non-neutral tone) receipts therefore become direct8-eligible during playback: fused single-pass 8-bit render replaces the indirect 16-bit pipeline; direct8-beats-proxy dispatch precedence is the EXISTING shipped contract (neutral receipts already did this). | x2 Sharp ~17 -> 20-23 (audit estimate: ~45ms indirect -> ~10-20ms fused); x1 +2-4; quality SHARPER at x1/x2 (full/half-res direct8 replaces half/quarter-res proxy lanes for these receipts). | interleaved base/r42 exe-swap A/B, x2 Sharp trio 2 reps/arm + x1 pairs; direct8 engagement cited via processed8_direct_path_frames from the same runs; x4/x8 no-regress singles; suite via manifest gate; VIEWED filmstrip sweeps + balance trace (pixel-affecting: lane character changes sharper by design) + x8 canary | rows 1-6, 13-15 target/watch | working tree on master 2c5017e5 (exe SHA at measurement) | Holds only if the shared-kernel parity families keep covering the preview-routed features (they do: byte-identity is mode-independent row math) and the sharper direct8 output is artifact/cast-free by eyes (the proxy softness users saw before was itself the round-3 trade; this REVERSES it for eligible receipts - filmstrips adjudicate). | FINAL gate exit 0 (observed=10 = exact manifest; one honest re-pin: StandardPreviewScaleTwoUsesQuarterResShadowsHighlightsByDefault now uses a sharpening-ineligible receipt so the preserved path-5 quarterres-SH composition stays exercised - SH receipts ride direct8 with export blur now; its mid-suite env leak had also been failing PlaybackProxyLevelFullDisablesPreviewCoresMidClip as collateral); VIEWED x2 filmstrips clean x3 caps + x8 canary clean x2 caps; balance trace truebase-vs-r42h arms <=0.99 warmCool / 0.48 greenAxis | IN-FLIGHT FINDINGS - THE VIEWED SWEEP CAUGHT THREE REAL DEFECTS the fps/content detectors were blind to (corrupt-render fps numbers DISCARDED): (a) x2 direct8 frames showed multi-band temporal/positional tearing on EVERY capture; root cause = the direct8/worker-indirect helpers hand the shared TLS rgb_u16 slot as the scaled-input buffer while the x2 quarter core ensures the SAME slot for its own q_rgb intermediate and upscales out of it - self-aliased read/write rendered near-flat garbage (3 distinct values per 100k px at threads=1; LATENT for any neutral-receipt x2 direct8 since round-3 item 5; the worker byte-identity tests could not catch it because reference and worker corrupt identically). FIX: dedicated mlv_ensure_thread_scaled_input_buffer TLS slot in all four helpers. (b) The SH quarterres blur lane (standard-x2) feeds the direct8 kernel content it misreads (whole frame collapses to lifted-black tan); only the dimension-based halfres/fullres blur lanes are PhaseE9-proven against the kernel. FIX: force_export_policy param on the shared blur helper - applyProcessingObject8 pins export policy, the 16-bit path keeps its scale-aware lanes. (c) The kernel wrote the LIVE presented buffer in place over ~10-20ms (indirect only exposes a ~2ms packdown window) - out-of-band paints sample torn row blocks. FIX: render to TLS u8 scratch + one-memcpy publish (self-elides for the worker). ALSO: x1 full-res direct8 preempting the half-res proxy sagged x1 13->9 (audit's +2-4 expectation was wrong), and the mean23 preview-decode regime (Playback quality=preview) sagged x2 14.9->11.4 because the direct8 input path lacks the cheap mean23 cores - BOTH scoped out via mlv_preview_direct8_input_is_cheap (x1-proxy + dual-ISO-non-HQ guards, mirrored in the prefetch worker); the serial-render band-aid and the intrin preview bypass (which silently fed AgX receipts to an AgX-less kernel - live correctness bug) are gone. HONEST NUMBERS (clean renders, interleaved truebase 27145698-era vs r42h 6D015ACC, 6 pairs x2 Sharp): 1327 17.9->20.0 / 17.9->21.3 (+15.4% mean), 1347 17.2->23.3 / 17.5->21.7 (+29.7%), 1446 17.2->22.7 / 17.9->20.4 (+22.8%); arm means 17.6->21.6 (+22.7%); all 12 verdicts PASS; x1 pairs parity (15.6/15.6, 14.9/15.2, 15.2/15.2; proxy lane d8=0); singles x4 24.4 / x8 23.3; env-aggressive x1 15.9, x2 wash (15.05 vs 15.6 means; final interleaved pair 15.6->16.4); mean23-regime parity restored (14.1 vs 15.2 single pair, x4 11.9 vs 11.8) | KEEP (x2 Sharp ~20.4-23.3 median = the audit's 20-23 target hit honestly; keeper bar >=15% met on 3/3 clips; quality also SHARPER at x2 for direct8-eligible receipts - half-res fused render replaces quarter-process+upscale; plus three latent correctness bugs fixed incl. one shipping since round-3 item 5) | wb-cf8d8b096b824c36 |
| 2026-06-13 | x1 + x2 all modes (round-4 item 3: render N+1 / present N pipelining) | ONE edit in MainWindow::drawFrameReady: dispatch the NEXT frame render (the same inline timerFrameEvent advance the post-present tail already does) immediately after the acquire + generation + forward-only guards pass and BEFORE the present work. Mechanism (state-machine audit first, per plan): the present span runs ON the GUI thread, so the render thread sat starved for its whole duration - the serialization was request STARVATION, not a lock (isIdle()/phase3WorkInFlightLocked already exclude Ready/Presenting slots; the 4-slot machinery, request deque, latest-by-serial acquire and forward-only guard already supported one frame rendering while another presents). The tail advance still runs and degrades to m_playbackFrameAdvancePending while the early render is in flight. | Hide the ~12-15ms present span behind the next render: x2 toward native, x1 +20-30% (audit estimate). | interleaved exe-swap A/B base=8643AA0A vs r43=8BEAEE8A (arms verified functionally), x2 + x1 trios; flickerBackJumps + frozenContentRuns machine gates on every run; VIEWED x1/x2 filmstrips (temporal-order risk class); suite manifest gate | rows 1-6, 13-15 | working tree on master a3deba5a | Holds only if presentation order stays monotonic (forward-only guard owns this; flicker gates pinned per run) and the early advance touches nothing the in-flight present reads (it is the EXISTING tail call moved earlier; present reads only the acquired slot buffers). | gate exit 0 (observed=10 = manifest); flicker=0 frozen=0 on ALL 14 runs; VIEWED x2 cap-034/cap-last + x1 cap-051 clean (no tearing, no banding, no order artifacts) | x2 interleaved: 1327 21.7->23.8 (+9.7%), 1347 22.2->23.3 (+5.0%), 1446 21.3->22.7 (+6.6%) - r43 medians 22.7-23.8 sit AT the 23.976 native ceiling (the gain is ceiling-capped, not lever-capped); x1 wash as predicted post-hoc (16.1->16.4 / 16.1->16.1 / 16.4->16.7: x1 is render-bound at ~40ms, so hiding the present span buys little); singles x4 22.7 / x8 22.7 / envaggr-x2 22.2 all healthy | KEEP (x2 Sharp ~22.7-23.8 = native-locked; the audit's "x2 ceiling near-native" reached; x1 +20-30% estimate REFUTED for the proxy lane - the pacing residual only converts to fps where render time fits the frame budget) | wb-b547f43c80f740b0 |
| 2026-06-13 | non-dual-ISO clips x1/x2 (round-4 item 4) | NOT ATTEMPTED - BLOCKED ON ASSET: relaxing the llrpHQDualIso gates in the x1/x2 proxy cores is a pixel-affecting change whose mandatory VIEWED sweep needs real standard-CFA footage; every clip on this machine (C:\temp\MLV, 13 files probed) is dual-ISO (avg_llrawproc_dual_iso_ms ~11 = live recon). A dual-ISO clip with recon forced off is NOT a valid stand-in (the bright/dark row pattern would alias in the downsample kernels and poison the visual verdict). | the same ~2x x1 lift for ordinary footage | n/a | n/a | n/a | n/a | n/a | n/a | ROADMAP (ask the user for a non-dual-ISO MLV; the change itself is small: a recon-skipping branch in the proxy cores) | wb-28d03a5385504698 (recorded in the closure block) |
| 2026-06-13 | quality + remainder closure (round-4 item 5) | Quality filmstrip session on shipped lanes + evidence-based adjudication of the LJ92/SIMD remainder + final standings + the round stop-condition check. | settle the audit's quality findings with eyes; decide if any remaining perf lever clears noise | VIEWED x1 proxy-vs-Full filmstrip pair (fine vertical detail: railing edges, hair, glass) on M16-1446; current-master standings from the R4-3 A/B battery | all rows | master 90451965 (exe 8BEAEE8A) | n/a (closure) | x1 proxy cap-052 VIEWED: no aliasing/zipper on railing edge, no chroma fringing, hair soft but clean; full-lane cap VIEWED sharp and clean; all 30+ filmstrips VIEWED across the round were artifact-free at display scale | LJ92 wider refill + SIMD gaps: REFUTED-BY-STANDINGS - x2/x4/x8 are native-locked (nothing to gain) and at x1 the ~2-3ms estimate sits under single-clip noise against a ~40ms render bound by dual-ISO recon (~10-11ms) + half-dim processing (~16.5ms) with raw reads already 95%+ prefetch-hidden (1.7-4.1ms avg) | ROUND-4 COMPLETE: stop condition met (x2/x4/x8 native-locked; remaining x1 levers below noise; remaining x1 speed steps are the shipped UI tradeoffs) | wb-28d03a5385504698 |
| 2026-06-13 | full-quality x1 + x2 + x1 Auto (round-4 item 6: user chose "x1 full" after the re-audit found full-quality x1 was NOT maxed) | Let PLAYBACK direct8 lanes use the cheap preview quarter-res shadows/highlights RBF instead of the export-policy half/full-res RBF. applyProcessingObject8 hardcoded force_export_policy=1 as a round-4 item 2 band-aid; the corruption that motivated it was the TLS scaled-input self-alias (fixed separately, same item), NOT the blur resolution - the blur_image is full imageX*imageY in every branch and the quarter/half sigmas are tuned to the SAME effective radius (sigma*width identical). New: sh_export_policy = !(preview_mode && processing_direct8_preview_quarterres_sh_enabled()); EXPORTS (preview mode off, R4-0b) keep the export policy and stay full quality. Kill switch MLVAPP_DISABLE_DIRECT8_PREVIEW_QUARTERRES_SH. | Cut the ~21ms full-quality-x1 SH-prep ~4x; lift the band-aid off x1 Auto / x2 too. | env-toggle A/B (ONE build, no stale-object risk), 30s sustained windows, both clips; suite manifest gate; VIEWED filmstrips on EVERY affected lane (band-aid lift is broad); same-frame mean-RGB tone check; no-regress singles | rows 1-6, 13-15 | master 90451965 (exe 1E7E8158) | Holds only if the quarter-res blur is tonally equivalent (it is: same-frame Delta ~0.6/255 uniform, identical warmCool/greenAxis) and no lane shows the R4-2 corruption (none do - that was the TLS alias). | suite gate exit 0 (=10 manifest); VIEWED x1-Full/x1-Auto/x2/x4/x8 all clean (no lifted-black-tan, no banding); same-frame tone Delta ~0.6/255 uniform NO color shift (the balance-trace |>6| flags were separate-run timecode misalignment, refuted by the controlled same-frame capture) | 30s A/B (qres vs half-band-aid): full-quality x1 10.6->11.6 (1327) / 11.2->11.9 (1446) = +6-9%, sh_prep 21->14.6ms, processed16 80->71ms; x2 Sharp 22.2->23.3 (1327, toward native) / 23.3 (1446); x1 Auto 15.4->16.1 / 15.9->16.4 (+3-4%, marginal - its SH input is already half-dim); x4 23.3 / x8 23.3 / aggressive-x2 22.7 no-regress; all frozen=0 flicker=0 (one x1auto-1446 verdict FAIL = single 308ms interval, the documented x1 >=250ms calibration trip, content-clean distinctHashes==presents) | KEEP (full-quality x1 ~11.6-11.9, the re-audit's stale 8-9/maxed claim corrected and a real quality-preserving win landed; undoes the over-broad R4-2 band-aid; exports protected) | wb-52e74c73a8b74f2b |
| 2026-06-13 | full-quality x1 (round-4 item 7: low-risk SH-RBF threading slice - user chose "low-risk slice first") | INVESTIGATION, NO CODE CHANGE. Goal: claw back part of the SH-blur cost by better-threading the RBF (it threads only ~1.4x vs the tone kernel's 6.8x), BYTE-IDENTICALLY. Diagnosis (probe split + RBFilterPlain.cpp read): the RBF core is ~94% of sh_prep (avg_sh_filter_quarterres_rbf_ms 100 of 106 under the probe; halfres=0 confirms the R4-6 quarter-res path); the horizontal passes, the L/R average, and the range table are ALREADY fully omp-parallel across rows/pixels; the ONLY under-threaded part is the vertical IIR, run as `runVerticalDown`||`runVerticalUp` under `#pragma omp parallel sections num_threads(2)` (RBFilterPlain.cpp:786) = 2 threads total. | byte-identical SH-RBF threading -> ~12-13 fps | n/a (investigation) | master 281e733e | n/a | FINDING: the vertical pass is NOT safely column-parallelizable byte-identically. It is a continuous LINEAR WALK ((height-1)*(width-1) iters, src_prev lagging exactly one row) with a load-bearing border quirk (inner loop is `x < width-1`, last column drifts; tail pixels are left for the opposing pass to fill - the "Ming version with better right border"). A clean per-column grid rewrite changes the output, so column-strip threading lands in the SAME tolerance-validation + parity-risk bucket as the SIMD rewrite, NOT a free win. | DEAD END (no free byte-identical threading win; the horizontal/average/range-table passes are already parallel, and the 2-threaded vertical IIR is quirky serial legacy code). The ONLY remaining full-quality-x1 lever is the parity-risky rewrite (SIMD tone kernel and/or a tolerance-validated RBF reimplementation), expected ~12.8-14 fps. Kill-category: implementation (the algorithm's exact output depends on a serial linear walk; safe parallelism needs a behavior-changing reimpl). | wb-072b14f4d9704835 |
