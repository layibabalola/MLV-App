...
- Live dashboard refresh should use `tools\closeout\write-repo-state.ps1 -RepoRoot . -Write -LatestOnly`, which updates only `repoStateLedger.latestPath` and avoids writing closeout history/audit noise on every poll.
- `webDashboardSpec` is read-only by default and `symbolic-action-request-only`: sticky `/closeout`, SSE with polling fallback, preserved scroll/focus/selection/expanded/history-filter state across refresh, repo-map/workflow/blocker/audit/rollback panels, and historical closeout browsing all read from repo-owned ledger/audit artifacts. `latest.json` is a display feed only, not rollback evidence.
- Rollback is feasible only through repo-owned evidence and Git-safe actions. `rollbackPolicy` prefers revert/recovery-branch restoration, path restore from snapshot, and preservation-ref promotion; requires a new work block, user approval, immutable source snapshot evidence, a `closeout-rollback-manifest.v1`, rollback plans, and recovery commands in mutating audits; and forbids `reset --hard`/force push unless the user explicitly requests it. Until a rollback actor validates the manifest, dashboard readiness must remain `read-only-no-actor`. Committed changes are usually highly recoverable; branch/worktree cleanup depends on preserved evidence; uncommitted foreign dirty paths remain manual.
