param(
    [Parameter(Mandatory)][ValidateSet('tick','status','observe-signal')][string]$Command,
    [string]$Demand = '',
...
if ($Command -eq 'tick') { if (-not $Demand) { throw 'Demand is required' }; $arguments += @('--demand', $Demand) }
