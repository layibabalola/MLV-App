param(
    [string]$RepoRoot = ".",
    [switch]$NoToast
)

$ErrorActionPreference = "Stop"

$root = [System.IO.Path]::GetFullPath((Join-Path (Get-Location) $RepoRoot))
$script = Join-Path $root "tools\agent-bridge\codex_bridge_reminder.ps1"
if (-not (Test-Path $script)) {
    throw "Bridge reminder script not found: $script"
}

$closeoutCli = Join-Path $root "tools\closeout\Invoke-CloseoutCli.ps1"
if (Test-Path $closeoutCli) {
    try {
        $freezeJson = & $closeoutCli -RepoRoot $root -Arguments @("remediation-freeze-status", "--action", "final-hook", "--audit") 2>$null
        $freezeText = ($freezeJson | Out-String).Trim()
        if ($freezeText) {
            $freeze = $freezeText | ConvertFrom-Json
            if ($freeze.active) {
                Write-Warning ("Final hook skipped during closeout remediation freeze: " + $freeze.markerPath)
                return
            }
        }
    } catch {
        Write-Warning ("Remediation freeze status check failed: " + $_.Exception.Message)
    }
}

function Assert-BridgeCliContract {
    param([Parameter(Mandatory=$true)][string]$Root)

    $cli = Join-Path $Root "tools\agent-bridge\agent_bridge.py"
    if (-not (Test-Path $cli)) {
        throw "Bridge CLI not found: $cli"
    }

    $probe = & py -3 $cli check-inbox --help 2>&1
    $exitCode = $LASTEXITCODE
    $text = ($probe | Out-String)
    if ($exitCode -ne 0 -or $text -notmatch "--agent" -or $text -notmatch "--format") {
        throw ("Bridge CLI contract failed: check-inbox help did not expose the expected JSON hygiene command. exit=" + $exitCode + " output=" + $text.Trim())
    }
}

Assert-BridgeCliContract -Root $root

if (Test-Path $closeoutCli) {
    try {
        & $closeoutCli -RepoRoot $root -Arguments @("bootstrap-response", "--hook-phase", "final", "--actor", "codex-final-hook") | Out-Null
    } catch {
        Write-Warning ("Response broker bootstrap skipped: " + $_.Exception.Message)
    }
}

$args = @(
    "-NoProfile",
    "-ExecutionPolicy",
    "Bypass",
    "-File",
    $script,
    "-WorkspaceRoot",
    $root,
    "-HookPhase",
    "final",
    "-SkipSessionWorktree",
    "-ProjectBucket",
    "mlv-app"
)

if ($NoToast) {
    $args += "-NoToast"
}

& powershell.exe @args
