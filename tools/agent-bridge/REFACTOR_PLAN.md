...
- Stage 6 restore-thread UUID gap: when `RestoreThreadId` is empty and Codex itself is the foreground app on a different or unprovable thread, targeted wake now fails closed with retryable exit 16 instead of navigating away and leaving the user on the bridge thread. Durable future work remains a real previous-thread identity primitive or a live-rendering app-server wake path.
