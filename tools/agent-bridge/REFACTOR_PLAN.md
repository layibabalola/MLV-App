...
- Targeted SendKeys thread proof/restore follow-up: the stale archived `desktop_thread_id` pin and stranded composer text are fixed in the May 3 wake-routing patch, but Codex Desktop still often exposes only the generic UIA root title `Codex`. User direction on 2026-05-03 prioritizes near-real-time delivery: when the target `ThreadId` is valid, wake may navigate to the paired bridge thread even if `RestoreThreadId` is unavailable; restore remains best-effort until a real previous-thread identity primitive or app-server wake path lands.
- Stage 6 restore-thread UUID gap: when `RestoreThreadId` is empty, the user lands on the bridge thread after wake with no guaranteed path back to their prior Codex thread. Durable fix: capture the restore UUID from UIA before navigation or move the handoff to a thread-addressable app-server wake path.
...
| 78 | Targeted SendKeys wake treats user UI state as delivery-priority: if the foreground is Codex on a different or unprovable thread and the target thread id is valid, the wake navigates to the target thread even when restore UUID is unavailable; restore is best-effort via `SetForegroundWindow`; defer only when the target thread id itself is unavailable or invalid | 19 |
