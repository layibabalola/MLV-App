...
- Targeted SendKeys thread proof/restore follow-up: the stale archived `desktop_thread_id` pin and stranded composer text are fixed in the May 3 wake-routing patch, but Codex Desktop still often exposes only the generic UIA root title `Codex`. Do not make title mismatch hard-fail or pass a fake `-RestoreThreadId`; instead add a real previous-thread identity primitive or move wake delivery to the app-server path before relying on automatic restore-to-previous-thread semantics.
