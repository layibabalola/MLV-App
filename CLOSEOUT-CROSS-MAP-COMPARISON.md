...
- This round hardens baseline-dirty fingerprint handling so partial promotion cannot hide changed current-work bytes inside retained foreign dirty.
- The practical next step is to extend this same fingerprint evidence into any future hunk-level split actor, if the repos decide to support hunk ownership rather than file-level blocking.
...
| `baseline-dirty-mixed-path-protection` | Baseline overlap blocks whole-file checkpoint and requires split or proof. | Baseline-dirty claimed paths and baseline-dirty paths whose status/content fingerprint changes after broker start become mixed or unowned, not auto-owned or retained foreign. | Clean-at-start checkpointing is allowed only for proven clean baseline paths. | Pre-existing dirty bytes cannot be captured as current work by claim alone, and changed baseline-dirty files cannot be silently pruned/retained as foreign dirty. | Core MUST | Decide whether hunk-level ownership is future extension or explicit non-goal. |
...
| `remediation-freeze` | DNG has config, docs, and freeze actor references; treats it as Max/optional for small repos. | MLV-App has freeze marker/env, hook blocking, audit packets, fresh-vs-stale dirty fingerprint checks, and removal quorum tests. | AdversarialLLM reports this as a gap. | Freeze is a safety brake when lifecycle automation is making state worse; stale markers may be cleared only by the repo-owned thaw actor after exact stale-evidence quorum. | Max SHOULD | Define portable blocker semantics when unsupported. |
