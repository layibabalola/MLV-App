...
**The full contract lives in [`docs/20-closeout-agent-contract.md`](docs/20-closeout-agent-contract.md)
and is BINDING. Read it before any closeout work** — it was moved out of this file on 2026-09-04
because at 34,560 bytes it was 66% of an entry-tier file that every session auto-loads.
Moving it changed no rule; the text there is verbatim.

The routine path, so an ordinary turn needs nothing else:
...
**Non-negotiables that are easy to get wrong, and that the contract expands:**

- Work blocks start from **clean `master`/target or an already approved and integrated base** —
  never stack on an unapproved sibling branch.
- Finalize is blocked until the content-review gate is satisfied: **CODEX appends a `HANDOFF` entry,
  then CLAUDE appends a LATER `REVIEW` entry that approves it.** Codex's own handoff does not
  release the gate. Both entries need an exact full-40-char `Range:` line and a bare `Verdict:`
  token; the approving entry also needs an allowlisted `Seat:` GUID.
- A target push non-fast-forward is a **recoverable race, never permission to force-push**: fetch,
  fast-forward only if the fetched ref proves it is a descendant, rerun finalize.
- Foreign dirty work is **retained and audited, never stashed or deleted**.
- Response/final hooks are **read-only** for managed session worktrees.
- Closeout completion is a **fixed point, not the first successful merge** — rerun detection until
  nothing new appears.

Everything else — bounded runners, the dashboard contract, rollback policy, repo-sweep, the
remediation freeze, evidence-preserving prune — is in the contract file.
