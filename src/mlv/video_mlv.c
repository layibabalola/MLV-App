...
/* Quarter-res x2 preview (Phase 4B-x2q): reconstruct Dual ISO at x4 (quarter)
 * resolution for an x2 playback request, then bilinearly upscale 2x to the x2
 * display size. Trades x4-level sharpness DURING PLAYBACK for ~4x cheaper recon
 * (measured ~7 ms vs ~15-40 ms on M16-1327), which keeps cadence smooth even
 * when the CPU is thermally throttled. Pause/scrub/export are unaffected - this
 * only runs on the x2 playback render path. Gated by env (default OFF until
 * validated); the dual-ISO 4-row phase is preserved by reusing the same
                                      ...
                                      int threads)
{
    ...
    if (scaleFactor == 2)
    {
        ...
        else if (mlv_quarterres_x2_preview_enabled()
              && mlv_render_scaled_rgb16_x2_quarter_preview_core(video, frameIndex, NULL, outputFrame, threads))
        {
            /* x4-resolution recon upscaled to x2 display: cheaper cadence, softer
             * while playing. Falls through to the half-res path on dim rejection. */
            return 1;
        }
